// AI Accelerate — V2 SECTION ONLY (#ai-accelerate-v2 on healthcare-v3.html).
//
// This is js/ai-accelerate.js with exactly three lines changed: the two root
// lookups and the panel lookup are scoped to #ai-accelerate-v2 instead of
// matching the first .rd-ai-accelerate / .rd-ai-toggle / [data-ai-panel] in
// the document. Nothing else differs, and nothing else should — if the
// original file gains a fix, regenerate this one from it rather than patching
// both by hand:
//
//   sed -e "s/document.querySelector('.rd-ai-accelerate')/document.querySelector('#ai-accelerate-v2')/" \
//       -e "s/document.querySelector('.rd-ai-toggle')/document.querySelector('#ai-accelerate-v2 .rd-ai-toggle')/" \
//       -e "s/document.querySelector('\[data-ai-panel\]')/document.querySelector('#ai-accelerate-v2 [data-ai-panel]')/" \
...
// loser, rename the survivor's id back to "ai-accelerate", and delete this
// file along with its <script> tag.
//
// ---------------------------------------------------------------------------
...
(function () {
  var section = document.querySelector('#ai-accelerate-v2');
  if (!section) return;
...
(function () {
  var toggle = document.querySelector('#ai-accelerate-v2 .rd-ai-toggle');
  var panel = document.querySelector('#ai-accelerate-v2 [data-ai-panel]');

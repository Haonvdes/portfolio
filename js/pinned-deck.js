/*
  Pinned card deck — one scroll gesture moves exactly one card.

  Used by web-3.html §03 (js/web3-frictions.js) and the home page's Selected
  Product Initiatives (js/initiatives.js). Call window.rdPinnedDeck(track, opts).

  The stage pins for one viewport while the track scrolls past it. CSS fades the
  old card out and the new one in; this script only moves `.is-active`. A snap
  ...
  The track gets `.is-live` only when the pinned version can run: wide, tall
  enough, and motion allowed. In every other case the page's CSS lays the same
  cards out statically. The track must have no vertical padding: the rest
  positions are measured from its border box.

  opts:
    stage     selector for the sticky 100vh child of the track
    cards     selector for the cards (they need ids if the rail uses
              aria-controls; otherwise rail rows are matched by order)
    rail      optional selector for the .rd-deck-index list
    countVar  optional custom property that receives the card count
    minWidth  default 1181
    minHeight default 640
*/
(function () {
  'use strict';

  window.rdPinnedDeck = function (track, opts) {
    opts = opts || {};
    var MIN_WIDTH = opts.minWidth || 1181;
    var MIN_HEIGHT = opts.minHeight || 640;
    ...
    if (!track) return;
    var stage = track.querySelector(opts.stage);
    var cards = Array.prototype.slice.call(track.querySelectorAll(opts.cards));
    if (!stage || cards.length < 2) return;

    var rail = track.querySelector(opts.rail || '.rd-deck-index');
    var rows = rail ? rail.querySelectorAll('li') : [];
    var items = cards.map(function (card, i) {
      var btn = rail && card.id
        ? rail.querySelector('button[aria-controls="' + card.id + '"]')
        : null;
      return btn ? btn.parentNode : rows[i] || null;
    });
    ...
    if (opts.countVar) track.style.setProperty(opts.countVar, cards.length);

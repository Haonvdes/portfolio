  ...
  .addEventListener('submit', async function (event) {
    ...
    if (fileInput.files.length > 0) {
      ...
      if (file.size > maxSize) {
        ...
        displayError(
          `File size (${fileSizeMB}MB) exceeds the 5MB limit. Please choose a smaller file.`
        );
        ...
        const uploadInstructions =
          document.getElementById('uploadInstructions');
      ...
      // Kiểm tra loại file
      const allowedTypes = ['application/pdf']; // Chỉ chấp nhận file PDF
      if (!allowedTypes.includes(file.type)) {
        displayError('Only PDF files are allowed');
        fileInput.value = '';
        return;
      }
    ...
    try {
      ...
      const checkResult = async () => {
        if (attempts >= maxAttempts) {
          throw new Error(
            'Analysis is taking longer than expected. Please check your email for results later.'
          );
        }
        ...
        try {
          const checkResponse = await fetch(
            `${RESPONSE_API_URL}?email=${encodeURIComponent(userEmail)}`
          );
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function displayFileInfo(file) {
    ...
    document.getElementById('fileIconContainer').innerHTML =
...
function displayError(errorMessage) {
  ...
  if (responseMessage) {
    ...
    responseMessage.textContent =
      'Oops! Something went wrong; check your information and try to load again.';
  }
...
function validateEmail() {
  ...
  const emailError =
    ...
    createErrorElement('emailError', userEmail);
...
function validateForm() {
  ...
  const requirementError =
    document.getElementById('requirementError') ||
  ...
  if (!jobDescription.value.trim() && fileInput.files.length === 0) {
    showError(
      requirementError,
      'Please provide either a job description or upload a file'
    );
